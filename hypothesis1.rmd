...
For example, the population structure
...
But what if we use instead more "productive" years?
...
The determination coefficient is even better than the one for model using only mean age (0.7647 vs 0.6537)
...
In three cases mean.age coefficient is similar
...
```{r}
plot(data.Poland$year, data.Poland$birth.rate, col = "blue", pch = 16, main = "Poland", cex = 1.5, xlim = c(1960, 2010), ylim = c(0.8, 2.2), xlab="mean.age", ylab="birth.rate")
```

The birthrate improved a little around the time the countries joined the EU, so population age might not be the only factor in the birthrate
